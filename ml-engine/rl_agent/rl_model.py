...
class RLModel:
    ...
    def __init__(self):
        try:
            ...
            # Running in a non-root environment (e.g., tests or dev machine)
            # where iptables is not accessible. Continue without firewall control.
            print('FirewallController unavailable (non-root). RLModel will not perform blocking actions.')
            self.fw = None
    ...
    def step(self, action: int, state: Dict[str, Any]) -> float:
        ...
        if action == 0:
            ...
            # Hard block (if firewall available)
            if self.fw:
